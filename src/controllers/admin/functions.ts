...
export const create = async (email: string, password: string): Promise<IAdmin> =>{
    const admin = new Admin({email: email, password: password});
    const result = await admin.save();
    return result;
}

export const login = async (email: string, password: string): Promise<null|string> =>{
    ...
    if (!passwordMatch)
        return null;
    return jwt.sign(admin.getTokenData(), tokenSecret,{expiresIn: tokenExpire});
}
