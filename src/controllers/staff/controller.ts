...
const validate = (method: string): Array<ValidationChain> => {
    switch (method) {
        ...
        case 'saveExpense':
            return [
                body("trans_date", "transaction date is required").exists().isISO8601().toDate(),
...
const createStaff: Array<RequestHandler> = [
    ...
    async (req: Request, res: Response) => {
        try{
            ...
            if(staff){
                return res.status(201).json({ message: "Created succesfully", staff: staff });
            }else{
                return res.status(500).json({ message: "Something went wrong. Contact support" });
            }
...
const saveExpense: Array<RequestHandler> = [
  ...
  async (req:Request, res: Response) => {
      try {
          ...
          staff.expenses.push({ trans_date: trans_date, description: description, cost: cost });
          staff.save();
...
const getAllExpenses: RequestHandler = async(req: Request, res: Response) => {
    try {
        ...
        if(admin) {
            ...
            return res.status(200).json({ message: "Expenses", expenses:staff.expenses });
...
export {createStaff, login, getAllExpenses,getAllStaff,getStaff,saveExpense};
